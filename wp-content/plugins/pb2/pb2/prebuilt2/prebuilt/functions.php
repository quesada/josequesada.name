...
/* Prebuilt functions based off of the wonderful K2 Functions Panel. Regards! */
...
// if we can't find prebuilt installed lets go ahead and install all the options that run pb2.  This should run only one more time for all our existing users, then they will just be getting the upgrade function if it exists.
...
function k2menu() {
	...
	add_submenu_page('themes.php', 'Customize Prebuilt', 'Customize Prebuilt', 5, $k2loc . 'functions.php', 'menu');
...
<?php if (isset($_POST['configela'])) : ?>
	...
	<div class="updated">
		...
		<p><?php _e('ELA Options for Prebuilt have been set'); ?></p>
...
<?php if (isset($_POST['Submit'])) : ?>
	...
	<div class="updated">
		...
		<p><?php _e('Prebuilt Options have been updated!'); ?></p>
...
.style1 {color: #background-color: #f7f7f7; color: #035676;}
...
<div style="width: 500px; padding: 5px; background-color: #E7F5FE; font-size: 12px; font-family: Arial, Helvetica, sans-serif; line-height: 20px; border: 1px dotted #1072A7; margin: 0 auto;">Welcome to the new Prebuilt 2 Admin Panel. We now make it even easier for you to set certain functions, including chosing alternative stylesheets. Of course, you can still do things the old fashion way, by exploring the clean Prebuilt code. And, make sure to make use of the <a href="http://www.devlounge.net/index.php?s=wordpress+customization">wordpress

  articles on devlounge</a>. Be sure to visit Prebuilt's home, <a href="http://www.devlounge.net/extra/prebuilt" class="style1">here</a>. <a href="http://feeds.feedburner.com/Devlounge" class="style1">(rss)</a> <a href="http://www.devlounge.net/forum" class="style1">(support)</a></div>



<h2><?php _e('Prebuilt 2 Admin'); ?></h2>
...
<form name="dofollow" action="" method="post">
  ...
  <tr valign="top">
		...
		<th scope="row"><?php echo __('Prebuilt Color Combos'); ?></th>
		...
		<td>
			...
			<p><small>Want to change prebuilt's colors without reworking the code yourself? Lucky for you, we've already taken care of it. Simple chose another style sheet below, and a new clean, matching stylesheet (and corresponding header) will replace your current one. Pretty nice huh?</small></p>
		...
		<th scope="row"><?php echo __('Layout Width Design'); ?></th>

		<td>

			<input name="widthtype" id="fixed" type="radio" value="1" <?php checked('1', get_option('k2widthtype')); ?> /> 

			<label for="fixed"><?php _e('Standard Width (default)') ?></label><br />
			...
			<small>Now you can set your own page width for Prebuilt. Be aware, setting the width to flexible will most likely break the header, so you'll have to create a new one, but that's the beauty of a clean coded template like Prebuilt.</small>
